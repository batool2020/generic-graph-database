...
{
private static void shortestPaths(GraphInterface<String> graph,
                               Vertex startVertex  )
//Writes the shortest distance from startVertex to every 
//other reachable vertex in graph.
{
Edge Edge;
Edge saveEdge;         // for saving on priority queue
int minDistance;
int newDistance;

PriQueueInterface<Edge> pq = new HeapPriQ<Edge>(20);   // Assume at most 20 vertices
String vertex;
QueueInterface<String> vertexQueue = new LinkedQueue<String>();

graph.clearMarks();
saveEdge = new Edge(startVertex, startVertex, 0);
pq.enqueue(saveEdge);
...
{
 Edge = pq.dequeue();
 if (!graph.isMarked(Edge.getToVertex()))
 {
   graph.markVertex(Edge.getToVertex());
   System.out.println(Edge);
   Edge.setFromVertex(Edge.getToVertex());
   minDistance = Edge.getDistance();
   vertexQueue = graph.getToVertices(Edge.getFromVertex());
   while (!vertexQueue.isEmpty())
   {
     ...
     if (!graph.isMarked(vertex))
     {
       newDistance = minDistance 
                     + graph.weightIs(Edge.getFromVertex(), vertex);
       saveEdge = new Edge(Edge.getFromVertex().getvertexType().toString(), vertex, newDistance);
       pq.enqueue(saveEdge);
     }
   }
 }
...
private static boolean isPathDF(GraphInterface<String> graph,
                             String startVertex, 
                             String endVertex    ) throws StackUnderflowException, QueueUnderflowException
...
private static boolean isPathBF(GraphInterface<String> graph,
                             String startVertex, 
                             String endVertex    ) throws QueueOverflowException, QueueUnderflowException
...
public static void main(String[] args) 
{
 ...
 GraphInterface<String> graph = new Graph<>(true, false);
 Vertex s0 = new Vertex("Atlanta   ");
 Vertex s1 = new Vertex("Austin    ");
 Vertex s2 = new Vertex("Chicago   ");
 Vertex s3 = new Vertex("Dallas    ");
 Vertex s4 = new Vertex("Denver    ");
 Vertex s5 = new Vertex("Houston   ");
 Vertex s6 = new Vertex("Washington");
 ...
 graph.addEdge(s0.getvertexId(), s5.getvertexId(),800);
 graph.addEdge(s0.getvertexId(), s6.getvertexId(), 600);
 graph.addEdge(s1.getvertexId(), s3.getvertexId(), 200);
 graph.addEdge(s1.getvertexId(), s5.getvertexId(), 160);
 graph.addEdge(s2.getvertexId(), s4.getvertexId(), 1000);
 graph.addEdge(s3.getvertexId(), s1.getvertexId(), 200);
 graph.addEdge(s3.getvertexId(), s2.getvertexId(), 900);
 graph.addEdge(s3.getvertexId(), s4.getvertexId(), 780);
 graph.addEdge(s4.getvertexId(), s0.getvertexId(), 1400);
 graph.addEdge(s4.getvertexId(), s2.getvertexId(), 1000);
 graph.addEdge(s5.getvertexId(), s0.getvertexId(), 800);
 graph.addEdge(s6.getvertexId(), s0.getvertexId(), 600);
 graph.addEdge(s6.getvertexId(), s3.getvertexId(), 1300);
